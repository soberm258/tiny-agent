...
_SEARCHER_CACHE: Dict[tuple[int, str], Searcher] = {}
...
def _get_searcher(db_name: str) -> Searcher:
    ...
    # 线程安全：Searcher 内部包含 SQLite 连接（BM25 SQLite 倒排索引），不能跨线程复用。
    # 因此这里按线程缓存 Searcher，避免触发 “SQLite objects created in a thread...” 报错。
    cache_key = (threading.get_ident(), str(db_name))
...
def rag_search(query: str, topk: int = 5, db_name: str = "law", is_hyde: bool = False) -> str:
    """
      在当前数据库中进行证据检索（默认策略：HyDE + RRF + rerank），返回带元数据的片段列表。" \
       ...
       "用户询问法律条例问题时，必须查找law库，而case库可选择作为案例补充使用" \
    """
    q = (query or "").strip()
    if not q:
