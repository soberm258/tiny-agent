...
_RE_ADD = re.compile(rf"^\s*之([{_CN_NUM}])\s*(.*)\s*$")
_RE_ATTACHMENT = re.compile(rf"^\s*附件([{_CN_NUM}]+)\s*(.*)\s*$")
_RE_LAW_TITLE_LINE = re.compile(r"^\s*((中华人民共和国|.*(省|自治区|直辖市|特别行政区)).{0,40}?(?:法典|宪法|法|条例|规定|解释))\s*$")
